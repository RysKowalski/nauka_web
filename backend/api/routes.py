...
from api import gra
from api.new_module_validation import validate_dict_structure
from api.save_new_module import save_new_module
from api import admin, login_stuff
...
def get_info():
    # Ścieżka do pliku
    file_path = os.path.join('data', 'nauka_questions.json')
...
def user_exist(user: dict):
	print(user)
	with open(os.path.join("data", "nauka_user_data.json"), "r") as plik:
