...
class Game:
    def __init__(self: Self, elements: list[str], name: str) -> None:
        ...
        user_data: tuple[Any, Any] = self.load_user_data(name, self.modules)
        self.max_points = user_data[0]
        chances = user_data[1]

        with open(
            os.path.join("./nauka_web_api", "backend", "data", "nauka_questions.json")
        ) as plik:
        ...
        data: dict = {
            "element_list": dict(
                zip(
                    ...
                    [item["chance"] for item in self.elements],
                )
            ),
    ...
    def move(self: Self, answer: bool, answer_time: float) -> None:
        ...
        weights = np.array([item["chance"] for item in self.elements]) / np.sum(
            [item["chance"] for item in self.elements]
        )
    ...
    def load_user_data(self: Self, name: str, elements: list[str]):
        ...
        file_path = os.path.join(
            "./nauka_web_api", "backend", "data", "nauka_user_data.json"
        )
        ...
        try:
            ...
            if "|".join(elements) in points_data:
                ...
                with open(
                    os.path.join(
                        "nauka_web_api", "backend", "data", "nauka_questions.json"
                    ),
                    "r",
                ) as plik:
    ...
    def save_user_data(self: Self) -> None:
        ...
        data[self.username]["|".join(self.modules)] = {
            "max_points": self.max_points,
            "chances": [item["chance"] for item in self.elements],
        }
